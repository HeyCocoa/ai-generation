...
public class WebScreenshotUtils{
    ...
    private static WebDriver initChromeDriver(int width, int height){
        try {
            // 自动管理 ChromeDriver，添加更多配置选项
            WebDriverManager.chromedriver()
                    .driverVersion("latest")  // 使用最新版本
                    .setup();
            
            // 配置 Chrome 选项
            WebDriver driver = getDriver(width, height);
            // 设置页面加载超时
            ...
            try {
                log.info("尝试使用系统PATH中的ChromeDriver");
                WebDriver driver = getDriver(width, height);
                driver.manage().timeouts().pageLoadTimeout(Duration.ofSeconds(30));
                driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(10));
                return driver;
            } catch (Exception e2) {
                log.error("手动初始化也失败", e2);
                throw new BusinessException(ErrorCode.SYSTEM_ERROR, 
                    "初始化 Chrome 浏览器失败: " + e.getMessage() + 
                    "。请检查：1) Chrome浏览器是否已安装 2) 网络连接是否正常 3) 系统权限是否足够");
            }
        }
    }

    private static WebDriver getDriver(int width, int height){
